...
class Settings(BaseSettings):
    ...
    def validate_required_keys(cls, values):
        ...
        model = llm.get('model') if isinstance(llm, dict) else llm.model
        if not model:
            return values

        if "openai" in model and not (llm.get('openai_api_key') if isinstance(llm, dict) else llm.openai_api_key):
            raise ValueError("OpenAI API key is required for OpenAI models")
        if "deepseek" in model and not (llm.get('deepseek_api_key') if isinstance(llm, dict) else llm.deepseek_api_key):
            raise ValueError("DeepSeek API key is required for DeepSeek models")
        if "gemini" in model and not (llm.get('gemini_api_key') if isinstance(llm, dict) else llm.gemini_api_key):
            raise ValueError("Gemini API key is required for Gemini models")
        if "groq" in model and not (llm.get('groq_api_key') if isinstance(llm, dict) else llm.groq_api_key):
            raise ValueError("Groq API key is required for Groq models")
        if "mistral" in model and not (llm.get('mistral_nemo_base_url') if isinstance(llm, dict) else llm.mistral_nemo_base_url):
